...
class ReadSequenceException(IOError):
    """Exception raised when sequence file cannot be read."""
...
def is_nucleotide_sequence(txt):
    ...
    :param txt: should be raw string in uppercase without whitespaces
    :return: True if txt is a valid nucleotide sequence
    """
...
def read_raw(txt):
    ...
    if not is_nucleotide_sequence(txt_without_whitespaces):
        raise ReadSequenceException(
            "Is it really single nucleotide sequence in one of the supported formats?"
        )
...
def read_fasta(txt):
    """
    Reads file containing FASTA-formatted sequence.
